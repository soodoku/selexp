...
s_data <- subset(data, !is.na(pk) & !is.na(year) & !is.na(age))

getPalette <- colorRampPalette(brewer.pal(9, "Set1"))

ggplot(s_data, aes(age, pk, colour = year)) +
  stat_smooth(geom = "line", se = FALSE, method = 'loess', alpha = .7, size = .4) +
  stat_smooth(aes(age, pk, color = "All"), se = FALSE, method = 'loess', size = 1) +
  scale_colour_manual(values = c(getPalette(13), "black")) + 
  labs(x = "Age", y = "Interviewer Assessed Political Knowledge") + 
  scale_x_continuous(breaks = seq(15, 125, 10), labels = seq(15, 125, 10)) +  
  scale_y_continuous(breaks = seq(0, 1, .1), labels = nolead0s(seq(0, 1, .1)), lim = c(0, 1)) +
  theme_minimal(base_size = 9) +
  theme(panel.grid.major = element_line(color = "#E0E0E0", size = .35, linetype = 3)) +
  theme(panel.grid.minor = element_line(color = "#F0F0F0", size = .35, linetype = 3)) +
  theme(axis.ticks = element_blank()) +
  theme(plot.title = element_text(color = "#525252", size = 10, vjust = 1.25)) +
  theme(axis.text.x = element_text(size = 12, color = "#636363", angle = 0)) +
  theme(axis.text.y = element_text(size = 12, color = "#636363")) +
  theme(axis.title.x = element_text(size = 14, color = "#636363", vjust = 0)) +
  theme(axis.title.y = element_text(size = 14, color = "#636363", vjust = 1.25)) +
  theme(plot.margin = unit(c(0.35, 0.2, 0.3, 0.35), "cm")) + 
...
ggsave(file = "selexp/figs/aging_curves.pdf", dpi = 450)
